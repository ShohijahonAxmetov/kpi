...
<!-- CARDS -->
<div class="container-fluid">
    <form method="post" action="{{ route($route_name . '.update', [$route_parameter => $faculty]) }}" enctype="multipart/form-data" id="add">
        @csrf
        @method('put')
        <div class="row">
            <div class="col-12">
                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="form-group">
                                    ...
                                    <select required class="form-control mb-4 @error('university_id') is-invalid @enderror" name="university_id">
                                        @foreach ($universities as $key => $item)
                                        <option value="{{ $item->id }}" {{ old('university_id', $faculty->university_id) == $item->id ? 'selected' : '' }}>{{ $item->title }}</option>
                                        @endforeach
                                    </select>
                                    @error('university_id')
